...
from utils.utils import Melding as meld, lookup_model_uri_from_modelverzoek_uri
...
from frontend.turtle_creator import TurtleCreator
...
def one_time_add_column_modelverzoek_status_to_database():
    query = "ALTER TABLE modelverzoeken ADD COLUMN modelverzoek_status VARCHAR(255);"
    try:
...
one_time_add_column_modelverzoek_status_to_database()
...
def upload_file(melding_manager):
    ...
    if (not file_extension.startswith(".xl")) and (file_extension != cn.LDM_EXTENSION):
        # and (file_extension != cn.XML_EXTENSION):
...
def update_modelverzoek(melding_manager, table='modelverzoeken'):
    ...
    if 'binary_data' in request.files and request.files['binary_data'].filename != '' and request.files['binary_data'] != 'undefined':
        ...
        if (not file_extension.startswith(".xl")) and (
                file_extension != cn.LDM_EXTENSION):  # and (file_extension != cn.XML_EXTENSION):
            return jsonify(
...
def lees_toetsingsrapport(melding_manager, table='toetsingsrapport'):
    ...
    try:
        ...
        opmerking = f"\nOphalen modelverzoek mislukt.\nDe inhoud van het modelverzoek '{primary_key_value}' kon niet worden opgehaald. Controleer of het modelverzoek nog bestaat. Mocht de fout zich blijven herhalen, dan kunt u hiervan een melding maken zodat het beheerteam u verder kan helpen."
        melding_manager.meld(ms.ERROR, opmerking)
        app.logger.exception(f"{e}: {''.join(traceback.format_stack())}")
...
def upload_toetsingsrapport(melding_manager, table='toetsingsrapport'):
    ...
    # upload_data = dict(request.form)
    # load binary_data to file_content
    ...
    with db_cursor:
        ...
        query = f"""
          ...
          """
        values = [primary_key, file_content, file_name]
