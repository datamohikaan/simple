select uuid as model_uuid
     , json_object('uuid',uuid
				  ...
				  json_object(
						...
						'functional_valid_until_is_inclusive', IFNULL(CAST(ANY_VALUE(m.mde_FunctionalUntilFieldIsInclusive) AS INTEGER), 0),
						...
						'technical_valid_until_is_inclusive', IFNULL(CAST(ANY_VALUE(m.mde_TechnicalUntilFieldIsInclusive) AS INTEGER), 0)
				  )				  
  ...
  left join namespaces_json  using(model_uuid)
